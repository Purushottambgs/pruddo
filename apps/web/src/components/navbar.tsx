      ...
      <div className="border-t border-slate-100 dark:border-slate-800">
        <div className="mx-auto flex max-w-7xl gap-0 overflow-x-auto px-4 sm:px-6 lg:px-8">
          {NAV_CATEGORIES.map((cat) => {
            const isActive = pathname + "" === cat.href || (pathname === "/search" && cat.href.includes(pathname));
            return (
              <Link
                key={cat.label}
                href={cat.href}
                className={`shrink-0 border-b-2 px-4 py-2 text-sm transition ${
                  isActive
                    ...
                    : "border-transparent text-slate-500 hover:border-indigo-400 hover:text-slate-900 dark:hover:text-slate-100"
                }`}
              >
                {cat.label}
              </Link>
